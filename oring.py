...
def trainAndPredict(regressors):
    ...
    for (model, name) in regressors:
        ...
        crossScoresMeanAE       = cross_val_score(model, X, y, cv=3, scoring='neg_mean_absolute_error')
        crossScoresMeanSE       = cross_val_score(model, X, y, cv=3, scoring='neg_mean_squared_error')
        # use this crossScoresMeanSLE      = cross_val_score(model, X, y, cv=3, scoring='neg_mean_squared_log_error')
        crossScoresMeanSLE      = cross_val_score(model, X, y, cv=3, scoring='neg_mean_squared_error')
        crossScoresMedianAE     = cross_val_score(model, X, y, cv=3, scoring='neg_median_absolute_error')
        crossScoresExplainedVar = cross_val_score(model, X, y, cv=3, scoring='explained_variance')
        crossScoresR2           = cross_val_score(model, X, y, cv=3, scoring='r2')
